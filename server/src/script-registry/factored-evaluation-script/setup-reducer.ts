...
export const setupReducer = (
  ...
  const newState = produce(state, draftState => {
    if (action.actionType === "SETUP_RUN") {
      draftState.randomSeedString = action.setupData.randomSeedString;
      const prngId = createSeededRandomIdGenerator(
        action.setupData.randomSeedString,
      );
      const id = prngId();
        ...
        question: [
          {
            ...
            text: action.setupData.rootLevelQuestion || "Root level",
          },
        ],
      ...
      draftState.experts = action.setupData.initialExperts;
    }
  });
